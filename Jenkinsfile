...
/* run the ansible script for spring */
            ...
            steps {
                sh '''
                   # replace dbserver in host_vars
                     ...
                     sed -i 's/192.168.56.101/20.56.30.217/g' ~/workspace/ansible/group_vars/appservers.yaml
                     sed -i 's/192.168.56.103/4.234.160.113/g' ~/workspace/ansible/group_vars/appservers.yaml
                '''
                sh '''
       ...
       stage('Deploy frontend') {
            steps {
                sh '''
                   # sed -i 's/dbserver/20.56.30.217/g' ~/workspace/ansible/host_vars/appserver-vm.yaml
                    export ANSIBLE_CONFIG=~/workspace/ansible/ansible.cfg
